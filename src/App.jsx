...
function StartScreen({ onStart }) {
  ...
  const wrap = {
    ...
    background:"linear-gradient(160deg,#1a0e04,#2a1608 40%,#0a1a0a)",
    fontFamily:FONT,
    color:"#e8d5a0", padding:"20px 16px", boxSizing:"border-box", overflowY:"auto",
  };

  return (
    <div style={wrap}>
      <div style={{ textAlign:"center", marginBottom:14 }}>
        <div style={{ fontSize:10, letterSpacing:6, color:"#8a7040", marginBottom:4 }}>-- STENALDERNS FOLK --</div>
        <div style={{ fontSize:32, fontWeight:"bold", letterSpacing:4, color:"#d4a030" }}>ÖSTERLEN</div>
        <div style={{ fontSize:10, letterSpacing:4, color:"#6a5a30", marginTop:3 }}>ca 8000 f.Kr.</div>
      </div>

      
      {step === 0 && (
        <div style={{ width:"100%", maxWidth:340 }}>
          <div style={{ fontSize:12, color:"#8a7850", textAlign:"center", lineHeight:1.7, margin:"8px auto 18px", fontStyle:"italic" }}>
            Österlen vaknar ur istiden. Välj spelläge.
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {[
              ["ai",   "🤖", "Mot datorn",   "En spelare mot AI-stammen."],
              ["2p",   "👥", "Pass & Play",  "Två spelare, en telefon."],
              ["2pai", "⚔️", "2p + AI",      "Två spelare och en AI-stam. Alliera er eller svika varandra."],
            ].map(([id,icon,lbl,desc]) => (
              <button key={id} onClick={() => { setMode(id); setStep(id==="2p" ? 2 : 1); }}
                style={{ width:"100%", background:"rgba(255,248,225,0.05)",
                  border:"2px solid rgba(180,140,40,0.35)",
                  borderRadius:10, padding:"14px 16px", cursor:"pointer",
                  fontFamily:"inherit", color:"#e8d5a0",
                  display:"flex", alignItems:"center", gap:14, textAlign:"left" }}>
                <div style={{ fontSize:28, flexShrink:0 }}>{icon}</div>
                <div>
                  <div style={{ fontSize:13, color:"#c8a030", fontWeight:"bold" }}>{lbl}</div>
                  <div style={{ fontSize:10, color:"#8a7850", marginTop:2, fontStyle:"italic" }}>{desc}</div>
                </div>
              </button>
            ))}
          </div>
      ...
      {step === 1 && (
        <div style={{ width:"100%", maxWidth:340 }}>
          <div style={{ fontSize:10, color:"#8a7040", letterSpacing:2, marginBottom:10, textAlign:"center" }}>{mode==="2pai" ? "VÄLJ AI-STAM" : "VÄLJ MOTSTÅNDARE"}</div>
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:14 }}>
            {[
              ["passive",    "🌿", "Skateholmsfolket",  "Mesolitiska jägare-samlare från Österlenens kust. Lever på havet, undviker konflikt och expanderar försiktigt."],
              ["balanced",   "🏺", "Trattbägarfolket", "Neolitiska bönder som byggde stendösar i Skane. Metodiska och envisa – expanderar stadigt och försvarar sig val."],
              ["aggressive", "⚔",  "Stridsyxefolket",   "Kom osterifraan ca 2800 f.Kr. Krigarfolk med snabba ror. Anfaller tidigt och ofta."],
            ].map(([id,icon,nm,desc]) => (
              <button key={id} onClick={() => setAiP(id)}
                style={{ width:"100%", background:aiP===id?"rgba(255,248,225,0.1)":"rgba(255,248,225,0.04)",
                  border:`2px solid ${aiP===id?GOLD:"rgba(180,140,40,0.25)"}`,
                  borderRadius:10, padding:"12px 14px", cursor:"pointer",
                  fontFamily:"inherit", color:"#e8d5a0", textAlign:"left",
                  display:"flex", alignItems:"center", gap:12 }}>
                <div style={{ fontSize:28, flexShrink:0 }}>{icon}</div>
                <div>
                  <div style={{ fontSize:13, color:"#c8a030", marginBottom:3, fontWeight:"bold" }}>{nm}</div>
                  <div style={{ fontSize:10, color:"#8a7850", lineHeight:1.5, fontStyle:"italic" }}>{desc}</div>
                </div>
              </button>
            ))}
          </div>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={() => setStep(0)} style={{ ...btnSecondary, flex:1, padding:"10px" }}>Tillbaka</button>
            {aiP && <button onClick={() => setStep(2)} style={{ ...btnPrimary, flex:2, padding:"10px" }}>{mode==="2pai" ? "Välj spelare" : "Välj karakter"}</button>}
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div style={{ width:"100%", maxWidth:400 }}>
          <div style={{ fontSize:10, color:"#8a7040", letterSpacing:2, marginBottom:8, textAlign:"center" }}>
            {step2 ? "SPELARE 2 VÄLJER" : mode==="2pai" ? "SPELARE 1 VÄLJER" : "VÄLJ DIN ANFÄDER"}
          </div>
          <input value={step2?p2name:pname}
            ...
            style={{ background:"rgba(255,248,225,0.08)", border:"1px solid rgba(180,140,40,0.3)",
              color:"#e8d5a0", padding:"6px 12px", borderRadius:4, fontFamily:"inherit",
              fontSize:12, marginBottom:10, width:"100%", textAlign:"center", boxSizing:"border-box" }}/>
            ...
            {CHARACTERS.filter(ch => !step2 || ch.id !== pchar?.id).map(ch => {
              ...
              return (
                <button key={ch.id} onClick={() => step2?setP2char(ch):setPchar(ch)}
                  style={{ background:chosen?"rgba(255,248,225,0.12)":"rgba(255,248,225,0.05)",
                    border:`2px solid ${chosen?ch.color:"rgba(180,140,40,0.3)"}`,
                    borderRadius:10, padding:"12px 10px", cursor:"pointer",
                    fontFamily:"inherit", color:"#e8d5a0", textAlign:"center" }}>
                  <div style={{ fontSize:24, marginBottom:4 }}>{ch.icon}</div>
                  <div style={{ fontSize:12, fontWeight:"bold", color:ch.colorLight, marginBottom:2 }}>{ch.name}</div>
                  <div style={{ fontSize:8, color:"#b0a070", letterSpacing:1, marginBottom:5, fontStyle:"italic" }}>{ch.title}</div>
                  <div style={{ fontSize:10, color:"#d0c090", lineHeight:1.5, marginBottom:8, fontStyle:"italic" }}>{ch.desc}</div>
                  <div style={{ background:`${ch.color}33`, border:`1px solid ${ch.color}66`, borderRadius:4, padding:"4px 6px", fontSize:9, color:"#f0e0a0" }}>
                    {ch.bonus}
                  </div>
                </button>
              );
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={() => { if(step2){setStep2(false);setP2char(null);} else setStep(mode==="ai"?1:0); }}
              style={{ ...btnSecondary, flex:1, padding:"10px" }}>Tillbaka</button>
            {!step2 && pchar && mode==="2p" && (
              <button onClick={() => setStep2(true)} style={{ ...btnPrimary, flex:2, padding:"10px" }}>Spelare 2</button>
            )}
            {!step2 && pchar && mode==="ai" && (
              <button onClick={begin} style={{ ...btnPrimary, flex:2, padding:"10px" }}>BÖRJA SPELA</button>
            )}
            {step2 && p2char && (
              <button onClick={begin} style={{ ...btnPrimary, flex:2, padding:"10px" }}>BÖRJA SPELA</button>
            )}
...
export default function ÖsterlenCiv() {
  ...
  function endTurn() {
    ...
    if (ev) { setTimeout(() => setPendingEvent(ev), 400); return; }
  ...
  return (
    <div style={{ fontFamily:FONT,
      ...
      {discoveryQueue.length>0 && (
        <DiscoveryModal site={discoveryQueue[0]} onClose={() => setDiscoveryQueue(q => q.slice(1))}/>
      )}
      {pendingEvent && (
